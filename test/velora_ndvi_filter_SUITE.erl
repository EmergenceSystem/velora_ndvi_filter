-module(velora_ndvi_filter_SUITE).
...
query_filter(Body) ->
    httpc:request(post,
        {"http://127.0.0.1:19213/agent/query", [], "application/json",
         binary_to_list(Body)},
        [{timeout, 10000}], [{body_format, binary}]).

start_stub() ->
    Dispatch = cowboy_router:compile([
        {'_', [{"/agent/query", velora_ndvi_filter_stub_h, []}]}]),
    {ok, _} = cowboy:start_clear(velora_ndvi_stub_listener,
                                 [{port, 18081}],
                                 #{env => #{dispatch => Dispatch}}).

stop_stub() ->
    catch cowboy:stop_listener(velora_ndvi_stub_listener),
    ok.
